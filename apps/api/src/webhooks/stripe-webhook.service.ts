...
export class StripeWebhookService {
    ...
    async processWebhook(rawBody: Buffer, signature: string): Promise<void> {
        ...
        const isNew = await this.webhookEventRepo.insertIfNew(event.id, event.type, event as any);
        if (!isNew) {
            this.logger.log(`Duplicate webhook event ${event.id} (${event.type}) — already recorded, skipping`);
            return;
        }
