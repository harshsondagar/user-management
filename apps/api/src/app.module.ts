...
@Module({
  imports: [
    ...
    ConfigModule.forRoot({
      isGlobal: true,
      envFilePath: isTestEnv ? 'apps/api/test/.env.test' : 'apps/api/.env',
      load: [configuration],
    }),
    ...
    TypeOrmModule.forRootAsync({
      ...
      useFactory: (config: ConfigService) => ({
        ...
        password:
          tEnv?.postgresPassword ?? config.get<string>('database.password'),
  ...
  providers: [
    ...
    { provide: APP_INTERCEPTOR, useClass: AttachUserContextInterceptor },
    {
      ...
      useFactory: () => new LogCleanupService('api'),
    },
  ],
})
